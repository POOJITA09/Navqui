...
const AttemptBlindQuiz = ({ match }) => {
		...
		(event) => {
			if (event.keyCode === 32) {
				...
				// For Quiz not accessible
					...
					speak('You have attempted quiz #p ')
				else {
					SpeechRecognition.startListening({ continuous: true })
					console.log('started listeng..')
	...
	useEffect(() => {
		const fetchQuiz = async () => {
			const res = await fetch('/API/quizz/join', {
				method: 'POST',
			...
			if (quizData.error) {
				...
				// For Quiz not accessible
				else if (quizData.error === 'ERR:QUIZ_ACCS_DENIED')
					speak('Access is not granted by the creator.')
				else if (quizData.error === 'ERR:QUIZ_ALREY_ATTEMPED')
					speak('You have already atempted this quiz.')
			} else {
	...
	useEffect(() => {
		...
		const temp = [
			...
			},
			{
				...
				callback: () => {
					speak(
						'Use following command to interact with application. Start Quiz to initialize the quiz. Repeat the current question command to repeat the focused question. Select or choose option number command for choosing the correct option. Next question to move to the next consecutive question and Previous Question to move to the previous consecutive question.'
					)
					resetTranscript()
				}
			},
			...
			},
			{
				...
				callback: (dummy, number) => {
					...
					} else speak('Question Number not recognize Kindly repeat it')
					resetTranscript()
				},
				matchInterim: true
			},
	...
	} else if (quizTitle === 'ERR:QUIZzzALREA_ATTEMPTED') {
